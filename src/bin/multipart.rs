...
type AWSObjectStore = Arc<AmazonS3>;
type LocalObjectStore = Arc<LocalFileSystem>;

async fn local_helper(object_store: Arc<dyn ObjectStore>, path: PathBuf, mut multipart: Multipart) -> ObjStoreResult<(), String> {
...
async fn upload_local_file_handler(State(object_store): State<LocalObjectStore>, multipart: Multipart) -> Result<Html<String>, (StatusCode, String)> {
    ...
    local_helper(object_store, upload_path, multipart).await.map(|_| Html("File uploaded".to_string()))
        .map_err(|e| {
            eprintln!("Failed to store file: {:?}", e);
            (StatusCode::BAD_REQUEST, "Failed to store file".to_string())
        })
}
async fn upload_aws_file_handler(State(object_store): State<LocalObjectStore>, multipart: Multipart) -> Result<Html<String>, (StatusCode, String)> {
    println!("Uploading file");

    aws_helper(object_store, upload_path, multipart).await.map(|_| Html("File uploaded".to_string()))
        .map_err(|e| {
            eprintln!("Failed to store file: {:?}", e);
            (StatusCode::BAD_REQUEST, "Failed to store file".to_string())
        })
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let aws_object_store: AWSObjectStore = Arc::new(aws);
    let object_store: LocalObjectStore = Arc::new(LocalFileSystem::new());
    // Build our application with a single route
    let app = Router::new()
        .route("/upload/local", post(upload_local_file_handler))
        .route("/upload/aws", post(upload_local_file_handler))
        .layer(
            ServiceBuilder::new()
