...
use object_store::{local::LocalFileSystem, ObjectStore, PutResult, Result as ObjStoreResult};
use std::env;
use std::error::Error;
...
async fn local_helper(
    object_store: &LocalFileSystem,
    path: PathBuf,
    mut multipart: Multipart,
) -> ObjStoreResult<(), String> {
    while let Some(field) = multipart
        .next_field()
        .await
        .map_err(|_| "Failed to read multipart field".to_string())?
    {
...
async fn upload_local_file_handler(
    ...
    local_helper(&object_store.local_object_store, upload_path, multipart)
        .await
        .map(|_| Html("File uploaded".to_string()))
        .map_err(|e| {
            eprintln!("Failed to store file: {:?}", e);
            (StatusCode::BAD_REQUEST, "Failed to store file".to_string())
        })
}
async fn upload_aws_file_handler(
    State(state): State<Arc<AppState>>,
    multipart: Multipart,
) -> Result<Html<String>, (StatusCode, String)> {
    let object_store = state.clone();
    ...
    aws_helper(&object_store.aws_object_store, upload_path, multipart)
        .await
        .map(|_| Html("File uploaded".to_string()))
        .map_err(|e| {
            eprintln!("Failed to store file: {:?}", e);
